import type { CapturedLink, DisplayMode, PageMetadata, RuntimeResponse } from '../types'
import { appendCapturedLink, getState, setCaptureSession, setMode } from '../storage/extensionStorage'
...
chrome.debugger.onEvent.addListener(async (source, method, params) => {
  ...
  const existing = state.history.find((item) => item.trackingKey === trackingKey)
...
chrome.runtime.onMessage.addListener((request: CaptureRequest, _sender, sendResponse) => {
  void (async () => {
    try {
      ...
      sendResponse({ ok: false, message: 'Unsupported message type.' } satisfies RuntimeResponse)
    } catch (error) {
