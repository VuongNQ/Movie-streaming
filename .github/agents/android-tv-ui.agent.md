...
- Preserve the repository's existing app structure unless the task explicitly calls for a migration.
...
- If Compose for TV is already in use for the target area, follow the existing app direction; otherwise keep changes consistent with the current Kotlin UI stack.
...
2. Check whether the feature is currently using Leanback, Views, or Compose and follow the existing pattern.
